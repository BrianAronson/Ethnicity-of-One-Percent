*1 - Demographic comparison (whole sample);
	Data Sipptemp;
		set sipptemp;
	...
	Proc Means mean Data=SippTemp;	
		var Age25to39 Age40to49 Age50to59 Age60to69 Age70P LessThanHS HSGraduate SomeCollege Bachelors AdvancedDegree female White Black Latino Other married widowed divorced separated nevermarried unemployed; 
		by year;
		Weight wpfinwgt;
		where refperson=1;
		run;
...
*2 - Demographic comparison (just rich);
	...
	Data richSCFTemp;
		...
		if age=>70 then Age70P=1; else Age70P=0;
	run;
	Proc Means mean Data=richSippTemp;	
		var Age25to39 Age40to49 Age50to59 Age60to69 Age70P LessThanHS HSGraduate SomeCollege Bachelors AdvancedDegree female White Black Latino Other married widowed divorced separated nevermarried unemployed; 
		by year;
		Weight wpfinwgt;
		where refperson=1;
		run;
	Proc Means mean Data=richSCFTemp;	
		var Age25to39 Age40to49 Age50to59 Age60to69 Age70P LessThanHS HSGraduate SomeCollege Bachelors AdvancedDegree female White Black Latino Other married unemployed;
		by year;
		weight wgt;
		run;
...
* 3 - All imputation variable comparison;
	Proc Means mean Data=richSippTemp;	
...
*Correlation Matrix;
 proc corr data=RichSCFTEMP;
var age boarder children	evermilitary	female	grandchild	HomeOwner	HouseHoldSize	InterestBank	IRAor401k	loggedassets	loggeddebt loggedCarDebt	loggedCreditCardDebtValue	loggedHomeDebt	loggedIraValue	loggedLifeInsuranceValue	loggedOtherAssetValue	loggedOtherRealEstateValue	loggedsalaryincome	loggedSavingsPlanValue	loggedSecuredDebtValue	loggedVehicleValue	loggedyearsatjob	Mortgages	NumberCarOwned	otherrelative	ownmotorcycle	ownnoncar	ownrv	parent	StockMutual	supportchild	year Fulltime Homemaker Parttime Retired Unemployed SFulltime SHomemaker SParttime SRetired SUnemployed White Black Latino Other LessThanHS HSGraduate SomeCollege Bachelors AdvancedDegree;
run;


WorkStatus SWorkStatus Marital Race Education

Fulltime Homemaker Parttime Retired Unemployed SFulltime SHomemaker SParttime SRetired SUnemployed White Black Latino Other LessThanHS HSGraduate SomeCollege Bachelors AdvancedDegree
